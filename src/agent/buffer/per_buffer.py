...
class PERBuffer(BaseBuffer):
    ...
    def encode_obs(self, obs, prediction=False):
        """Normalizes (divides by 255) and converts to tensor. If prediction=True, adds batch dimension."""
    ...
    def save_buffer(self, buffer_dir, game, run):
        ...
        with gzip.open(observation_path, 'wb') as f:
            np.save(f, self.obs_buffer[:, -1]) # (n, c, h, w) # type: ignore
            
        with gzip.open(action_path, 'wb') as f:
            np.save(f, self.act_buffer) # (n,) # type: ignore

        with gzip.open(reward_path, 'wb') as f:
            np.save(f, self.rew_buffer) # (n,) # type: ignore
            
        with gzip.open(terminal_path, 'wb') as f:
            np.save(f, self.done_buffer) # (n,) # type: ignore
