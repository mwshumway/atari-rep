...
class WandbTrainerLogger(object):
    def __init__(self, cfg, init_wandb=True):
        self.cfg = cfg
...
class WandbAgentLogger(object):
    def __init__(self, cfg, init_wandb=True):
        ...
        dict_cfg = OmegaConf.to_container(cfg, resolve=True, throw_on_missing=True)
        if cfg.agent.resume:
            assert len(cfg.agent.run_ids)==cfg.world_size
            self.run_id = cfg.agent.run_ids[cfg.rank]
            cfg.agent.ckpt_path = cfg.agent.ckpt_paths[cfg.rank]
        else:
            self.run_id = wandb.util.generate_id()
            print(f"Rank {cfg.rank}, running with wandb run_id {self.run_id}")
    ...
    def save_dict(self, save_dict, name):
        # save checkpoint with premade dict
        path = f'./materials/agent/{self.cfg.project_name}/{self.cfg.group_name}/{self.cfg.exp_name}_{self.run_id}/{name}'
        _dir = os.path.dirname(path)
        if not os.path.exists(_dir):
            os.makedirs(_dir)
        
        # Only save latest, but still label with step number
        ckpt_step = save_dict['global_step']
        last_name = name.replace(f'ckpt_step_{ckpt_step}', f'ckpt_step_{ckpt_step - self.cfg.agent.ckpt_freq}')
        last_path = f'./materials/agent/{self.cfg.project_name}/{self.cfg.group_name}/{self.cfg.exp_name}_{self.run_id}/{last_name}'
