"""
Currently, all games are put into the same index space. This was done
since the heads are created (0-50) based on game_idx, so need to keep this consistent
when we run online RL to make sure we access the right head for each game.
"""
...
import cv2
from .base import *
from src.utils.class_utils import save__init__args
...
class AtariEnv(BaseEnv):
    ...
    def __init__(self,
        ...
        self._game = re.sub(r'(?<!^)(?=[A-Z])', '_', game).lower()
        ...
        if self._game in game_list:
            ...
            raise ValueError(f"Game {self._game} not found in ATARI_RANDOM_SCORE list.")
        ...
        self._max_start_noops = int(max_start_noops)
        self._frame_skip = int(frame_skip)
        self._clip_reward = bool(clip_reward)
        self._episodic_lives = bool(episodic_lives)
        self._frame = int(frame)
        self._repeat_action_probability = float(repeat_action_probability)
        self.reset()
        

    def seed(self, seed=None, id=0):
        _, seed1 = seeding.np_random(seed)
        if id > 0 and seed is not None:
            seed = seed*100 + id
        self.np_random, _ = seeding.np_random(seed)
        # Derive a random seed. This gets passed as a uint, but gets
        # checked as an int elsewhere, so we need to keep it below
        # 2**31.
        _, seed2 = seeding.np_random(seed1 + 1)
        seed2 = seed2 % (2**31)
    ...
    def reset(self):
        ...
        if self._max_start_noops > 0:
            for _ in range(self.np_random.integers(1, self._max_start_noops + 1)):
                self.ale.act(0)
                if self._check_life():
                    self.reset()
    ...
    def _update_obs(self, action):
        ...
        img = cv2.resize(self._max_frame, (self.imagesize, self.imagesize), interpolation=cv2.INTER_LINEAR)
        if len(img.shape) == 2:
            img = img[np.newaxis]
        else:
    ...
    def get_action_meanings(self):
        return [ACTION_MEANING[int(i)] for i in self._action_set]
...
ATARI_RANDOM_SCORE = {
    'air_raid': 579.25,
    'alien': 227.8, # near-ood
    'amidar': 5.8, 
    'assault': 222.4, # near-ood
    'asterix': 210.0, # near-ood
    'asteroids': 719.1, 
    'atlantis': 12850.0, 
    'bank_heist': 14.2, 
    'battle_zone': 2360.0, 
    'beam_rider': 363.9, # near-ood
    'berzerk': 123.7, 
    'bowling': 23.1, 
    'boxing': 0.1, 
    'breakout': 1.7, 
    ...
    'pitfall': -229.4, 
    'pong': -20.7, # near-ood
    'pooyan': 371.2, # near-ood
    'private_eye': 24.9,
    'qbert': 163.9, 
    'riverraid': 1338.5, # near-ood
    'road_runner': 11.5, 
    'robotank': 2.2, 
    'seaquest': 68.4, # near-ood
    'skiing': -17098.1, 
    'solaris': 1236.3, 
...
ATARI_HUMAN_SCORE = {
    'air_raid': 3325.0,
    'alien': 7127.7, # near-ood
    'amidar': 1719.5,
    'assault': 742.0, # near-ood
    'asterix': 8503.3, # near-ood
    'asteroids': 47388.7,
    'atlantis': 29028.1,
    'bank_heist': 753.1,
    'battle_zone': 37187.5,
    'beam_rider': 16926.5, # near-ood
    'berzerk': 2630.4,
    'bowling': 160.7,
    'boxing': 12.1,
    'breakout': 30.5,
    ...
    'private_eye': 69571.3,
    'qbert': 13455.0,
    'riverraid': 17118.0, # near-ood
    'road_runner': 7845.0,
    'robotank': 11.9,
    'seaquest': 42054.7, # near-ood
    'skiing': -4336.9,
    'solaris': 12326.7,
...
ATARI_DQN50M_SCORE = {
    'air_raid': 7479.50,
    'alien': 2484.49, # near-ood
    'amidar': 1207.74,
    'assault': 1525.24, # near-ood
    'asterix': 2711.41, # near-ood
    'asteroids': 698.37,
    'atlantis': 853640.00,
    'bank_heist': 601.79,
    'battle_zone': 17784.84,
    'beam_rider': 5852.42, # near-ood
    'berzerk': 487.48,
    'bowling': 30.12,
    'boxing': 77.99,
    'breakout': 96.23,
    ...
    'private_eye': -16.04,
    'qbert': 10117.50,
    'riverraid': 11638.93, # near-ood
    'road_runner': 36925.47,
    'robotank': 59.77,
    'seaquest': 1600.66, # near-ood
    'skiing': -15824.61,
    'solaris': 1436.40,
...
# Filter score dicts to only include the games we have data for (in GAMES)
ATARI_RANDOM_SCORE = {k: ATARI_RANDOM_SCORE[k] for k in GAMES}
ATARI_HUMAN_SCORE = {k: ATARI_HUMAN_SCORE[k] for k in GAMES}
ATARI_DQN50M_SCORE = {k: ATARI_DQN50M_SCORE[k] for k in GAMES}