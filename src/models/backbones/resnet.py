...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        out = F.relu(self.norm1(self.conv1(x))) # type: ignore
        out = F.relu(self.norm2(self.conv2(out))) # type: ignore
        out = self.norm3(self.conv3(out)) # type: ignore
        out += self.shortcut(x)
        out = F.relu(out)
        return out
...
class ResNet(BaseBackbone):
    ...
    def __init__(self, 
        ...
        super(ResNet, self).__init__(in_shape, action_size)
        f, c, h, w = in_shape
        self.in_channel = f * c
        
        self.conv1 = nn.Conv2d(self.in_channel, 64, kernel_size=7, 
                               stride=2, padding=3, bias=False)
        self.norm1 = init_normalization(channels=64, norm_type=norm_type)
        self.stem = nn.Sequential(
            self.conv1,
            self.norm1, # type: ignore
        ...
        self.layer1 = self._make_layer(block, int(64*width_multiplier), num_blocks[0], stride=1, norm_type=norm_type)
        self.layer2 = self._make_layer(block, int(128*width_multiplier), num_blocks[1], stride=2, norm_type=norm_type)
        self.layer3 = self._make_layer(block, int(256*width_multiplier), num_blocks[2], stride=2, norm_type=norm_type)
        self.layer4 = self._make_layer(block, int(512*width_multiplier), num_blocks[3], stride=2, norm_type=norm_type)
