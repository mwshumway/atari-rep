...
class MHSpatialMLPNeck(BaseNeck):
    ...
    def forward(self, x, idx=None):
        ...
        # Rep. candidate 1: pooled before spatial embedding + norm
        rep1 = self.pool(x)
        rep1 = rearrange(rep1, 'n c 1 1 -> n c')
        rep1 = self.norm(rep1)  # type: ignore
        rep1 = rearrange(rep1, '(n t) c -> n t c', t=t)
        ...
        # Rep. candidate 2: pooled after spatial embedding + norm
        rep2 = rearrange(x, '(n t) c -> n t c', t=t)

        # run through MLP
        x = self.mlp[0](x) # first layer of MLP
        x = self.mlp[1](x) # ReLU

        # Rep. candidate 3: after first layer of MLP
        rep3 = rearrange(x, '(n t) d -> n t d', t=t)

        x = self.mlp[2](x) # second layer of MLP
        x = self.mlp[3](x) # ReLU

        x = rearrange(x, '(n t) c -> n t c', t=t)

        # Rep. candidate 4: after second layer of MLP, final output 
        rep4 = x
        
        info = {
            "rep_candidate_1": rep1,
            "rep_candidate_2": rep2,
            "rep_candidate_3": rep3,
            "rep_candidate_4": rep4,
        }
