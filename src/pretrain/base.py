...
from src.common.train_utils import get_grad_norm_stats, is_main_process
from src.common.losses import SoftmaxFocalLoss
from src.common.vis_utils import visualize_plot
from einops import rearrange
import torch.distributed as dist

from src.probing.utils import extract_representations
...
class BaseTrainer():
    ...
    def evaluate(self, epoch_num) -> dict:
        eval_logs = {}

        # Probing
        # let's see, I've got self.eval_loader. I'll want to probe on this.
        # we'll run 3 different probes: action, reward, and value.
        # so let's extract the representations for the entire eval set first.
        collate_fn = lambda x: self._collate(x, f=self.cfg.frame)
    
        stack_of_batches = extract_representations(self.model, self.eval_loader, self.device, collate_fn)

        reps = stack_of_batches['obs_neck']
        acts = stack_of_batches['act']
        rews = stack_of_batches['rew']
        rtgs = stack_of_batches['rtg']
        game_ids = stack_of_batches['game_id']


        # Make probing datasets
        from src.probing.datasets.action import ActionProbeDataset
        from src.probing.datasets.reward import RewardProbeDataset
        from src.probing.datasets.value import ValueProbeDataset
        action_dataset = ActionProbeDataset(model=self.model, dataloader=self.eval_loader, device=self.device, feature_extractor="neck", cfg=self.cfg, extract=False)
        reward_dataset = RewardProbeDataset(model=self.model, dataloader=self.eval_loader, device=self.device, feature_extractor="neck", cfg=self.cfg, extract=False)
        value_dataset = ValueProbeDataset(model=self.model, dataloader=self.eval_loader, device=self.device, feature_extractor="neck", cfg=self.cfg, extract=False)

        action_dataset.representations = reps
        action_dataset.targets = acts
        action_dataset.game_ids = game_ids

        reward_dataset.representations = reps
        reward_dataset.targets = rews
        reward_dataset.game_ids = game_ids

        value_dataset.representations = reps
        value_dataset.targets = rtgs
        value_dataset.game_ids = game_ids

        # Action probing
        from src.probing.probes.action import ActionProbe
        action_probe = ActionProbe(cfg=self.global_cfg, device=self.device, action_size=self.cfg.action_size)
        act_eval_metrics, act_loss_hist, act_metrics_hist = action_probe.train(action_dataset)

        # Reward probing
        from src.probing.probes.reward import RewardProbe
        reward_probe = RewardProbe(self.global_cfg, device=self.device)
        rew_eval_metrics, rew_loss_hist, rew_metrics_hist = reward_probe.train(reward_dataset)

        # Value probing
        from src.probing.probes.value import ValueProbe
        value_probe = ValueProbe(self.global_cfg, device=self.device)
        val_eval_metrics, val_loss_hist, val_metrics_hist = value_probe.train(value_dataset)

        # Evaluation metrics reported after probe training, on all of eval dataset
        eval_logs['action_probe_acc'] = act_eval_metrics['accuracy']
        eval_logs['action_probe_f1'] = act_eval_metrics['f1_macro']
        eval_logs['reward_probe_acc'] = rew_eval_metrics['accuracy']
        eval_logs['reward_probe_f1'] = rew_eval_metrics['f1_macro']
        eval_logs['value_probe_r2'] = val_eval_metrics['r2']

        _act_data = [[i, act_loss_hist[i], act_metrics_hist[i]["accuracy"], act_metrics_hist[i]["f1_macro"]]
                        for i in range(len(act_loss_hist))]
        _rew_data = [[i, rew_loss_hist[i], rew_metrics_hist[i]["accuracy"], rew_metrics_hist[i]["f1_macro"]]
                        for i in range(len(rew_loss_hist))]
        _val_data = [[i, val_loss_hist[i], val_metrics_hist[i]["r2"]]
                        for i in range(len(val_loss_hist))]
        
        act_hist_table = wandb.Table(columns=["probe_step", "loss", "accuracy", "f1-macro"], data=_act_data)
        rew_hist_table = wandb.Table(columns=["probe_step", "loss", "accuracy", "f1-macro"], data=_rew_data)
        val_hist_table = wandb.Table(columns=["probe_step", "loss", "r2"], data=_val_data)

        eval_logs["action_probe_loss_curve"] = wandb.plot.line(act_hist_table, "probe_step", "loss", title=f"Action Probe Training Loss, Epoch {epoch_num}")
        eval_logs["action_probe_acc_curve"] = wandb.plot.line(act_hist_table, "probe_step", "accuracy", title=f"Action Probe Training Accuracy, Epoch {epoch_num}")
        eval_logs["reward_probe_loss_curve"] = wandb.plot.line(rew_hist_table, "probe_step", "loss", title=f"Reward Probe Training Loss, Epoch: {epoch_num}")
        eval_logs["reward_probe_acc_curve"] = wandb.plot.line(rew_hist_table, "probe_step", "accuracy", title=f"Reward Probe Training Accuracy, Epoch: {epoch_num}")
        eval_logs["value_probe_loss_curve"] = wandb.plot.line(val_hist_table, "probe_step", "loss", title=f"Value Probe Training Loss, Epoch: {epoch_num}")
        eval_logs["value_probe_r2_curve"] = wandb.plot.line(val_hist_table, "probe_step", "r2", title=f"Value Probe Training r2, Epoch: {epoch_num}")
